...
@section('container')
<link rel="stylesheet" href="{{asset ('/assets/css/page/users/pln/plntab.css')}}">
<section class="page-section">
    <div class="container">
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <h2 class="mb-3 font-weight-bold">Table of Staff PLN</h2>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="7" class="text-center py-sm-5">
                                <i class="fas fa-sad-tear"></i> {{$plns}} User | 404 NOT FOUND
                            </td>
                        </tr>
                        @endif
                        @foreach ( $data_pln as $pln )
                        <tr>
                            <td>{{$loop->iteration}}</td>
                            <td>{{$pln->nama_depan}}</td>
                            <td>{{$pln->nama_belakang}}</td>
                            <td>{{$pln->no_telp}}</td>
                            <td style="width: 300px;">{{$pln->kelurahan}}, {{$pln->kecamatan}},{{$pln->kota_kab}}, {{$pln->alamat}}, {{$pln->kode_pos}}</td>
                            <td>
                                <a href="/pln/{{ $pln->id }}/edit" class="badge badge-success py-2 px-1" data-toggle="tooltip" data-placement="bottom" title="Edit">
                                    <i class="fas fa-edit mx-sm-2"></i>
                                </a>
                                <a href="/pln/{{ $pln->id }}/destroy" class="badge badge-danger py-2 px-1" data-toggle="tooltip" data-placement="bottom" title="Delete" onclick="return confirm('Are you sure you want to delete this data ?')">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">Form New PLN User</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="/pln/create" method="POST">
                        @csrf
                        <div class="row">
                            <div class="col-sm">
                        ...
                        <div class="form-group">
                            <input type="text" name="levels" class="form-control" id="exampleFormControlInput4" value="PLN" hidden>
                        </div>
                        <div class="form-group">
